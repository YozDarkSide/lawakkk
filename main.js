...
const starts = async (XNdy = new WAConnection()) => {
    ...
    gam = img
    jadinya = await XNdy.prepareMessage(from, gam, MessageType.image)
    buttonMessagesI = {
    ...
    XNdy.sendMessage(from, buttonMessagesI, MessageType.buttonsMessage)
  }

    fs.existsSync('./Ndyie.json') && XNdy.loadAuthInfo('./Ndyie.json')
    XNdy.on('connecting', () => {
        start('2', 'Menghubungkan...')
    })
 XNdy.on('open', () => {
        success('2', 'Tersambung, Thanks To, Ndyie Botz, Melcanz YT, Caliph, Yogi Prasetya, Dhani, Zextz, Gavin, Wira')
    })
    await XNdy.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Ndyie.json', JSON.stringify(XNdy.base64EncodedAuthInfo(), null, '\t'))

    XNdy.on('chat-update', async (message) => {
        require('./XNdy.js')(XNdy, message, _welkom)
    })
XNdy.on("group-participants-update", async (anu) => {

    const isWelkom = _welkom.includes(anu.jid)
    try {
      groupMet = await XNdy.groupMetadata(anu.jid)
      ...
      console.log(anu)
      try {
        pp_user = await XNdy.getProfilePicture(mem)
      } catch (e) {
        pp_user = "https://telegra.ph/file/c9dfa715c26518201f478.jpg"
      }
      try {
        pp_grup = await XNdy.getProfilePicture(anu.jid)
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60"
      }
      if (anu.action == "add" && mem.includes(XNdy.user.jid)) {
        XNdy.sendMessage(anu.jid, "Gue Bot Anjing Ketik #menu")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(XNdy.user.jid)) {
        mdata = await XNdy.groupMetadata(anu.jid)
        memeg = mdata.participants.length
        num = anu.participants[0]
        let v = XNdy.contacts[num] || { notify: num.replace(/@.+/, "") }
        anu_user = v.vname || v.notify || num.split("@")[0]
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa Follow Sosial Media Ownerku\n• Youtube : https://youtube.com/channel/UC9WgqpVGWW87R3RWjmAxZgg\n• Instagram : https://instagram.com/ndyie_botz\n• Tiktok : https://tiktok.com/@Rendy.\n• Github : https://github.com/Ndyie`
        ...
        sendButImage(mdata.id, wel, "©Created : Ndyie Botz", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(XNdy.user.jid)) {
        mdata = await XNdy.groupMetadata(anu.jid)
        num = anu.participants[0]
        let w = XNdy.contacts[num] || { notify: num.replace(/@.+/, "") }
        ...
        sendButImage(mdata.id, out, "©Created : Ndyie Botz", buff, but)
      }
      if (anu.action == "promote") {
        const mdata = await XNdy.groupMetadata(anu.jid)
        anu_user = XNdy.contacts[mem]
        num = anu.participants[0]
        try {
          ppimg = await XNdy.getProfilePicture(
        ...
        XNdy.sendMessage(mdata.id, teks, MessageType.text)
      }
